...
var openAnimation = function() {
		...
		if ( explore.length > 0 ) {	
			...
			explore.waypoint( function( direction ) {
				...
				setTimeout(function() {
						explore.find('.to-open-animation').each(function( k ) {
							var el = $(this);
							
							setTimeout ( function () {
								el.addClass('open-animation');
					...
					$(this.element).addClass('open-animation');
...
function cycleText(){
  ...
  setTimeout(function(){
	...
	$(function(){
		...
		if(isMobile){
			$("#moveForward_btn").hide();
			$("#moveBackward_btn").hide();
		}
		if(!isMobile)
		{
			var view = $(".home");
			var sliderLimit = $(".home").width();
				var move ="20%";
			var count=1;
			
			$('#home_ID').on('mousewheel', function(event) {
			    
			    var currentPosition = parseInt(view.css("left"));
				    console.log(count + "---" + currentPosition);
			    
			    if(event.deltaY==-1)
			    {		
			    	if (count <= 5)
			    	{ 
			    		view.stop(false,true).animate({left:"-="+move},{ duration: 1000});
			    		count=count+1;
			    	}
			    }
			    if(event.deltaY==1)
			...
			$("#moveForward_btn").click(function(){
			 	if (count <= 5)
			    	{ 
			    		view.stop(false,true).animate({left:"-="+move},{ duration: 1000});
			    		count=count+1;
			...
			$("#moveBackward_btn").click(function(){
